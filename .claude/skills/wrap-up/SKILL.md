---
name: wrap-up
description: Use when ending a working session in the a2ui-github daily-work harness — to commit the session, mark a sub-task or phase done, merge a finished sub-task to main, or save a handoff before stepping away.
---

# wrap-up

Closes a session. See [`_dev/harness/daily-workflow.md`](../../../_dev/harness/daily-workflow.md) for the loop this fits.
...
- The handoff doc is what `pick-up-task` reads on a `[WIP]` resume — keep it scannable, not a transcript.
