/**
 * Task-8.4 canvas chrome baselines: the new floating/full-width furniture composed over a
 * replayed stage — live Back button, the parked banner, and the open history list. Driven off
 * the recorded-beat replay path (`?beat=8,1&instant`), zero LLM. The stage content and the
 * "data as of" clock are masked: beat surfaces carry live RelativeTimes and the capture time
 * is per-run, so their pixels would rot the baselines — the chrome is what these guard.
 */
