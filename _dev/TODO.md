...
- `yarn format:check` is red on a clean `main`, so `verify:all` cannot go green for anyone (found during 7.9, untouched — out of that task's scope). Five files are formatting drift; the sixth is a real defect: `client/src/chat/ChatView.css:21` ends `background: var(--bgColor-inset);/` — a stray slash postcss refuses to parse, apparently arriving with 4bc9658.
- Diff viewer (stretch)
