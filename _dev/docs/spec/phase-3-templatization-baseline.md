...
Init is authored complete and structured to survive Phase 4–7 corrections. Its phases: collect the 8 tokens, demo-arc interview, run the fill + author SPEC §3 prose, enable the harness + superpowers plugins, guarded self-delete. The mechanical placeholder-fill is a **standalone non-interactive script** that substitutes the known 8-token set across contents+paths, renames the token-named directories, and asserts no `{{...}}` remain — no scan step, no hardcoded per-file edit list. The fill script is smoke-tested this phase (3.4); the Claude-only steps (demo-arc interview, §3 authoring, plugin enablement) are authored with concrete commands but their **live end-to-end run is deferred** to the post-Phase-7 dogfood; self-delete is authored and structurally guarded so the smoke test never triggers it.
