...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `TrailingAction: React.FC<React.PropsWithChildren<ChildrenPropTypes>>`, where
...
- **Component-level description (→ `catalog.json` `description`):** A slot for an action control
  placed after the title, visible on regular and wide viewports by default.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | An action control placed after the title. |
