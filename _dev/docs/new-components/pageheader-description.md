...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `Description: React.FC<React.PropsWithChildren<ChildrenPropTypes>>`, where
...
- **Component-level description (→ `catalog.json` `description`):** A container for supporting text
  or metadata shown below the title.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | Supporting text or metadata shown below the title. |
