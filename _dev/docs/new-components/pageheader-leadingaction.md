...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `LeadingAction: FCWithSlotMarker<React.PropsWithChildren<ChildrenPropTypes>>`, where
  `ChildrenPropTypes = { className?, hidden?: boolean | ResponsiveValue<boolean> }`. Implementation
  `PageHeader.js`: `hidden` default `hiddenOnNarrow` (visible on regular and wide only).
- **Component-level description (→ `catalog.json` `description`):** A slot for an action control
  placed before the title, visible on regular and wide viewports by default.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | An action control placed before the title. |
| `hidden` | carry | no | `responsive(z.boolean())` | Whether the slot is hidden, optionally per viewport width (`narrow`/`regular`/`wide`). Unset applies the library default: hidden on narrow, visible on regular and wide. |
