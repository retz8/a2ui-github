...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `ContextBar: React.FC<React.PropsWithChildren<ChildrenPropTypes>>`, where
...
- **Component-level description (→ `catalog.json` `description`):** A container for additional
  contextual content shown in the context area, visible on narrow viewports by default.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | Additional contextual content shown in the context area. |
