...
- **Component-level description (→ `catalog.json` `description`):** An icon shown after a tree
  item's label.
...
|---|---|---|---|---|
| `child` | carry (required) | yes (← `children`) | `ComponentId` | The icon shown after the item's label. |
...
|---|---|---|---|
| `tree-view-visuals` | `child` (trailing icon) | item with `LeadingVisual`→`Icon(file)` and `TrailingVisual`→`Icon(dot)`; each with `label` | yes |

Shared with `tree-view-leadingvisual.md` — one fixture exercises both visuals on one item.
...
None. `TrailingVisual` carries no `Action`.
