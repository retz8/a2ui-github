...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `ContextAreaActions: React.FC<React.PropsWithChildren<ChildrenPropTypes>>`, where
...
- **Component-level description (→ `catalog.json` `description`):** A container for action controls
  shown in the context area, visible on narrow viewports by default.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | Action controls shown in the context area. |
