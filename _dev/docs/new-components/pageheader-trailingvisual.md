...
- **Real prop surface resolved from:** `@primer/react` type declarations at
  ...
  `TrailingVisual: React.FC<React.PropsWithChildren<ChildrenPropTypes>>`, where
...
- **Component-level description (→ `catalog.json` `description`):** A slot for a visual element,
  such as a label or badge, placed after the title.
...
|---|---|---|---|---|
| `children` | carry (optional) | yes | `ChildList` | A visual element, such as a label or badge, placed after the title. |
