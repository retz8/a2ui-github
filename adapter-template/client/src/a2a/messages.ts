 ...
 *
 * The deterministic server returns a single completed Task whose status
 * message carries the A2UI as version-tagged DataParts. A direct agent
 * Message reply is also supported.
 *
 * When your agent generates progressively, call this per chunk over
 * `sendMessageStream`'s stream events (Task | TaskStatusUpdateEvent) instead
 * of against one terminal Task.
 */
