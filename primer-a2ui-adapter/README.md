...
- **`CATALOG`** — the assembled `Catalog` object: every component
  ...
  message processor.
- **`CATALOG_ID`** — the catalog URI. It is the same string as the `catalogId`
  field inside `catalog.json`; a surface stamped with it declares "render me
  with this catalog".
...
// …feed A2UI messages into the processor, then render:
<A2uiSurface surface={surface} />;
```

**In the agent** (Python, outside the yarn workspace), the same contract is
consumed as the document rather than the code: `agent/catalog_common` locates
`primer-a2ui-adapter/catalogs/v0.9.1/catalog.json` by path and uses it for
...
```bash
yarn workspace primer-a2ui-adapter run build      # tsc → dist/
yarn workspace primer-a2ui-adapter run typecheck
yarn workspace primer-a2ui-adapter run test       # vitest
yarn workspace primer-a2ui-adapter run lint
```
